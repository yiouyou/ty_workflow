# python ci-i2i-202303.py -txtoutdir ..\tmp\output\ci-i2i\ci\test\ -imgoutdir ..\tmp\output\ci-i2i\i2i\test\ -userconf ..\tmp\input\ci-i2i\test\zz.json
import json
...
import calendar
import argparse
from _util_202305 import api_upscale, api_reload_model, api_interrogate
from _util_202305 import api_i2i, api_t2i
from _util_202305 import parse_creative, parse_sd1x_lora, parse_ctrl11_units
from _util_202305 import b64_img, is_human, random_human, merge_prompt
from _util_202305 import create_caption_txt, read_txt, create_blank_png
from _util_202305 import run_cmd
...
# if user_sys_conf in _i2i_sys_conf_zz:
#     if_original = userconf_json["if_original"]
...
def user_t2i_ctrl11(model_key, _parm, _ctrls):
    global _scripts_lora_0, _scripts_lora_3, _scripts_lora_4
    pos_prompt = merge_prompt(_pos_prompt, _parm["caption"])
    neg_prompt = _neg_prompt
    if _parm["if_sd15"]:
    ...
    override_settings = {}
    # override_settings["CLIP_stop_at_last_layers"] = 2
    controlnet_units = parse_ctrl11_units(_ctrls, user_sys_conf)
...
def user_i2i_ctrl11(model_key, _imgs, _parm, _ctrls):
    global _scripts_lora_0, _scripts_lora_3, _scripts_lora_4
    pos_prompt = merge_prompt(_pos_prompt, _parm["caption"])
    neg_prompt = _neg_prompt
    if _parm["if_sd15"]:
    ...
    # pprint(_payload)
    print(f"user_i2i_ctrl11: {_parm['width']}*{_parm['height']}")
    api_i2i(_payload, _out, _parm["model_name"])
    out_png = f"{_out}_ignore.png"
    upload_png = f"{_out}.png"
...
def user_i2i_mask(model_key, _imgs, _parm):
    ...
    _add = {
        ...
        "resize_mode": 0, # Just resize (0), Crop and resize (1), Resize and fill (2), Just resize (latent upscale) (3)
        "mask_blur": 4,
        "inpainting_mask_invert": 0, # inpaint masked (0), inpaint not masked (1)
        "inpainting_fill": 2, # fill (0), original (1), latent noise (2), latent nothing (3)
...
def run_which(model_key, _imgs, _parm):
    ...
    if _imgs["content"]:
        if user_sys_conf in _i2i_sys_conf:
            if _ctrls:
                user_i2i_ctrl11(model_key, _imgs, _parm, _ctrls)
            else:
            ...
            _parm["caption"] = _content + ", " + _style
            user_i2i_ctrl11(model_key, _imgs, _parm, _ctrls)
        elif user_sys_conf in _i2i_sys_conf_xp:
            ##### set shuffle as content
            _ctrls = {}
            _ctrls["shuffle"] = _imgs["content"]
            # _content, _style = api_interrogate(_imgs['content'])
            ...
            ##### set softedge as content
            _ctrls = {}
            _ctrls["softedge"] = _imgs["content"]
            _parm["caption"] = ""
            user_i2i_ctrl11(model_key, _imgs, _parm, _ctrls)
        ...
        if user_sys_conf in _t2i_sys_conf_txt:
            ...
            if _imgs["scribble"]:
                ...
                user_t2i_ctrl11(model_key, _parm, _ctrls)
            elif _imgs["softedge"]:
                ##### 与涂鸦大小一致
                ty_img = Image.open(_imgs["softedge"])
                if user_pos_prompt =="":
                    _content, _style = api_interrogate(_imgs['softedge'])
                    _parm["caption"] = _content
                ...
                print(
                    f"ERROR: for t2i_txtimg, {user_sys_conf} must have scribble/openpose/skeleton input!"
                )
...
if os.path.exists(_userconf):
    ...
    # _dir = "E:\\_Ai\\_workflow\\tmp\\input\\ci-i2i"
    # print(_dir)
